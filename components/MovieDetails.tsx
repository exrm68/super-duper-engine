        ...
        <div className="flex-1 overflow-y-auto no-scrollbar relative z-10 pt-[35vh]">
            <div className="px-6 pb-24 bg-gradient-to-t from-black via-black to-transparent min-h-[65vh]">
              ...
              >
                ...
                {/* ✅ NEW: Detailed Metadata Row */}
                {(movie.duration || movie.fileSize || movie.audioLanguage || movie.subtitles) && (
                ...
                <div className="min-h-[200px]">
                    ...
                    {isSeries && activeTab === 'episodes' && (
                        <div>
                           ...
                           {/* Episode List - ENHANCED WITH THUMBNAILS */}
                           <div className="space-y-3">
                              {currentEpisodes.length > 0 ? currentEpisodes.map((ep, index) => {
                                ...
                                return (
                                  ...
                                  >
                                    {/* Episode Thumbnail - Premium or Default */}
                                    <div 
                                      onClick={() => !isLocked && handleWatch(ep.telegramCode)}
                                      className={`relative rounded-lg overflow-hidden shrink-0 ${isLocked ? 'cursor-not-allowed' : 'cursor-pointer hover:scale-105'} transition-transform ${ep.thumbnail ? 'w-24 h-14' : 'w-14 h-14'}`}
                                    >
                                        {ep.thumbnail ? (
                                          // Premium: Episode specific thumbnail
                                          <>
                                            <img src={ep.thumbnail} className="absolute inset-0 w-full h-full object-cover" alt={ep.title} />
                                            {isLocked && <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center">🔒</div>}
                                            {!isLocked && <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 flex items-center justify-center transition-all">
                                              <Play size={20} fill="white" className="text-white opacity-0 group-hover:opacity-100 transition-opacity drop-shadow-lg" />
                                            </div>}
                                          </>
                                        ) : (
                                          // Default: Movie thumbnail with play icon
                                          <>
                                            <img src={movie.thumbnail} className="absolute inset-0 w-full h-full object-cover opacity-40 blur-[2px]" alt="" />
                                            {isLocked ? (
                                              <div className="absolute inset-0 flex items-center justify-center text-2xl">🔒</div>
                                            ) : (
                                              <Play size={18} fill="white" className="text-white z-10 drop-shadow-lg relative" />
                                            )}
                                          </>
                                        )}
                                    </div>
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-1">
                                            <h4 className="text-sm font-bold text-gray-200 group-hover:text-gold transition-colors truncate pr-2 leading-tight">
                                                {ep.number}. {ep.title}
                                                {isLocked && (
                                                  <span className="ml-2 text-[8px] bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full border border-yellow-500/30">
                                                    COMING SOON
                                                  </span>
                                                )}
                                            </h4>
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        <button
                                          ...
                                          className={`p-2.5 border rounded-lg transition-all ${
                                            isLocked 
                                              ...
                                              : 'bg-[#1a1a1a] hover:bg-[#252525] border-white/10 active:scale-95'
                                          } group/btn`}
                                        ...
                                        <button 
                                          ...
                                          className={`p-2.5 border rounded-lg transition-all ${
                                            isLocked 
                                              ? 'bg-gray-800 border-gray-700 cursor-not-allowed opacity-40'
                                              : 'bg-gold/10 hover:bg-gold/20 border-gold/30 active:scale-95'
                                          }`}
                                          title={isLocked ? "Coming Soon" : "Watch Episode"}
                                        >
                                          <Play size={14} fill={isLocked ? "gray" : "#FFD700"} className={isLocked ? "text-gray-600" : "text-gold"} />
                                        </button>
                                    </div>
                                  </motion.div>
                                );
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-2 overflow-hidden">
                                  {movie.screenshots.slice(0, 8).map((screenshot, idx) => (
                                    <div 
                                      key={idx}
                                      className="relative aspect-video rounded-lg overflow-hidden bg-[#111] border border-white/5 group cursor-pointer"
                                      onClick={() => {
